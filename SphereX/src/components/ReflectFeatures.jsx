...
export default function ReflectFeatures() {
  return (
    <div className="bg-[#020012] text-white py-16 px-4 text-center">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-y-12  max-w-5xl mx-auto before:content-[''] relative group before:absolute before:bottom-0 before:left-0 before:w-full before:h-[1px] before:pointer-events-none 
  ...
  before:bg-gradient-to-r before:from-[rgba(255,255,255,0)] before:via-[rgba(255,255,255,0.2)] before:to-[rgba(255,255,255,0)]">
        {features.slice(0, 3).map((feature, idx) => (
          <div key={idx} className="p-[24px_0_32px] relative group gap-[2px]
  ...
  after:bg-gradient-to-t after:from-[rgba(255,255,255,0)] after:to-[rgba(255,255,255,0.2)]">
          {feature.icon}
            <h3 className="font-medium text-sm sm:text-lg text-white mt-[20px]">{feature.title}</h3>
            <p className="text-sm sm:text-[16px] text-gray-400">{feature.description}</p>
          </div>
        ))}
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-12  max-w-3xl mx-auto ">
        {features.slice(3).map((feature, idx) => (
          <div key={idx} className="p-[24px_0_32px]  border-b-[#ffffff3d] relative group 
  after:content-[''] after:absolute after:top-0 after:right-0 after:w-[1px] after:h-full after:pointer-events-none 
  after:duration-500 after:ease-[cubic-bezier(0.3,0.8,0,1)] 
  after:bg-gradient-to-t after:from-[rgba(255,255,255,0)] after:to-[rgba(255,255,255,0.2)]">
            {feature.icon}
            <h3 className="font-medium text-sm sm:text-base text-white mt-[20px]">{feature.title}</h3>
            <p className="text-sm sm:text-[16px] text-gray-400">{feature.description}</p>
          </div>
        ))}
      </div>
